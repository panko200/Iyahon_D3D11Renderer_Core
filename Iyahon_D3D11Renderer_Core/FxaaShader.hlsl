...
float4 PS_Fxaa(PSInput input) : SV_Target
{
    ...
    float lumaN  = Luma(gTex.Sample(gSampler, uv + float2( 0, -1) * rcpFrame).rgb);
    float lumaS  = Luma(gTex.Sample(gSampler, uv + float2( 0,  1) * rcpFrame).rgb);
    float lumaE  = Luma(gTex.Sample(gSampler, uv + float2( 1,  0) * rcpFrame).rgb);
    float lumaW  = Luma(gTex.Sample(gSampler, uv + float2(-1,  0) * rcpFrame).rgb);
    ...
    float lumaNW = Luma(gTex.Sample(gSampler, uv + float2(-1, -1) * rcpFrame).rgb);
    float lumaNE = Luma(gTex.Sample(gSampler, uv + float2( 1, -1) * rcpFrame).rgb);
    float lumaSW = Luma(gTex.Sample(gSampler, uv + float2(-1,  1) * rcpFrame).rgb);
    float lumaSE = Luma(gTex.Sample(gSampler, uv + float2( 1,  1) * rcpFrame).rgb);
    ...
    float lumaEnd1 = Luma(gTex.Sample(gSampler, uv1).rgb) - lumaLocalAverage;
    float lumaEnd2 = Luma(gTex.Sample(gSampler, uv2).rgb) - lumaLocalAverage;
    ...
    for (int i = 0; i < 8; i++)
    {
        if (!reached1) { uv1 -= offset; lumaEnd1 = Luma(gTex.Sample(gSampler, uv1).rgb) - lumaLocalAverage; reached1 = abs(lumaEnd1) >= grad1; }
        if (!reached2) { uv2 += offset; lumaEnd2 = Luma(gTex.Sample(gSampler, uv2).rgb) - lumaLocalAverage; reached2 = abs(lumaEnd2) >= grad2; }
